...
//	settings.condition_3_adjusted_likes = [12000, 9999999]; //1
  ...
  //'Heather','Georgeee'
    settings.likes_by = ['Heather','Georgeee','Sarah','Arjen','Jane','AncaD','Lauren','John','Ky'];
    settings.Dislikes_by = ['John','AncaD','Sarah','Arjen','Jane','Georgeee','Lauren','Heather','Ky'];
  }
  ...
  function DeaktiviereLike(){
	...
	alert("The time has expired. Please return to the questionnaire."); }, 2000);
  }
  ...
  function init_task() {
		...
		for(var i=0; i<times.length; i++)
  		{
			...
			if(times[i]==10000){
				themsg = usernames[i] + " liked your profile description";
	...
	$('.usersDislikes').each(function(){
		if(times==11111){
					var that = $(this);
					themsg = usernames + " disliked your profile description";
					setTimeout(function(themsg) {
					that.text(parseInt(that.text()) + 1);
